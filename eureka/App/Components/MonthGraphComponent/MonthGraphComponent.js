...
import {MonthBrowser} from '../../Chart/AppUtility/DateTimeUtils';
...
// import {INSTANT_CALIBRATE_STATE} from '../../constants/AppDataConstants';
...
let _tempDateBrowser = new MonthBrowser();
...
export function MonthGraphComponent({vitalType, navigation}) {
  const [containerState, setContainerState] = useState({
    ...initState,
    dateFragment: temp1,
    previousDateFragment: temp2,
  });
  const [dateBrowser, setDateBrowser] = useState(new MonthBrowser());
...
/**
 * Graph and its associated functional component. We wrap this in the higher order MonthGraphComponent functional component.
 */
function MonthGraphContainer({
  startTs,
  endTs,
  previousStartTs,
  previousEndTs,
...
function convertToViewData(
  ...
  try {
    return convertMonthlyGeneralData(
      dailyVitalData,
      previousDailyVitalData,
      startTs,
      endTs,
...
function getTabReadingInfo(summary) {
  ...
  let trendText =
    noOfDaysRangeImproved === 0
      ? t('trends.month.same')
      : noOfDaysRangeImproved < 0
      ? t('trends.month.less',{value:-1 * noOfDaysRangeImproved })
      : t('trends.month.more',{value:1 * noOfDaysRangeImproved });
  tabReadingData.jsx = (
    <>
